...
class GrndetailController extends Controller
{
	...
	public function actionCreate($pp_purordnum, $vGrnNumber)
	{
		...
		if(isset($_POST['Grndetail']))
		{
			...
			if($model->save()){
		        ...
		        $purchaseorddt = Grndetail::model()->findByAttributes(array('cm_code'=>$cm_code, 'id'=>$id ));
		
		        $pp_grnqty = $purchaseorddt->im_RcvQuantity;
		        
		        $sql = "UPDATE pp_purchaseorddt SET pp_grnqty='{$pp_grnqty}' WHERE cm_code='{$cm_code}' AND pp_purordnum='{$pp_purordnum}' ";
				$command  = Yii::app()->db->createCommand($sql);
                $command->execute(); 
		        //echo $model2->pp_grnqty;
		        //exit();
		        
		        $this->redirect(array('Create','id'=>$model->id, 'pp_purordnum'=>$pp_purordnum, 'vGrnNumber'=>$vGrnNumber, ));
		      }
	 ...
	 * Manages all models.
	 */
	public function actionGrnDetailAdmin($im_grnnumber)
	{
