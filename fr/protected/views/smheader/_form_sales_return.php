...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$(".items tr").click(function() {
		    ...
		    id=id+1;
		    total = parseInt(total)+parseInt($.trim(tableData[6]));

		    var tax1 = document.getElementById("sm_total_tax_amt").value;
			taxamount = parseInt(tax1) + parseInt($.trim(tableData[3])) * (parseInt($.trim(tableData[4])) * parseInt($.trim(tableData[5]))/100);
		    
			var preValue = parseInt($.trim(tableData[6]));
			var preVat = parseInt($.trim(tableData[5]));
			var preQty = parseInt($.trim(tableData[4]));
			var preRate = parseInt($.trim(tableData[3]));
		   $("#test").append(
		    	"<tr><td><input value='"+$.trim(tableData[0])+"' style='width: 180px;' readonly > </td><td><input name='cm_code[]' value='"+$.trim(tableData[1])+"' style='width: 80px;' readonly ></td><td><input name='sm_batchnumber[]' value='' style='width: 80px; text-align: center;'></td><td><input name='sm_expdate[]' value='' style='width: 100px;'></td><td><input name='sm_unit[]' value='"+$.trim(tableData[2])+"' style='width: 110px;' readonly ></td><td><input name='sm_rate[]'  value='"+$.trim(tableData[3])+"' id='rate_"+id+"' style='width: 80px;text-align: right;' readonly ></td><td><input name='sm_quantity[]' value='"+$.trim(tableData[4])+"' onchange='price_multiply(this.id)' id='"+id+"' style='width: 60px;text-align: center;'> <input type='hidden' id='check_"+id+"' value='"+$.trim(tableData[4])+"'  > </td><td><input name='sm_tax_rate[]' id='tax_"+id+"'  value='"+$.trim(tableData[5])+"' style='width: 60px;text-align: center;' readonly ></td><td><input name='sm_lineamt[]'  id='price_"+id+"' value='"+$.trim(tableData[6])+"' style='width: 100px; text-align: right;' readonly ></td></td><td> <span onclick='deleteRow(\""+ preValue + "\", \"" + preVat + "\", \"" + preQty + "\", \"" + preRate + "\", this)' style='background:orange; color:white; border-radius: 3px; cursor: pointer; padding: 1px 3px; margin-left: 5px;'> x </span> </td></tr>");
	...
	function deleteRow(preValue, preVat, preQty, preRate, row)
	{
		$(row).closest('tr').remove();

		var preTotal =document.getElementById("sm_totalamt").value;
		total = parseInt(preTotal) - parseInt(preValue);
		document.getElementById("sm_totalamt").value = total;

		var preTaxAmt = document.getElementById("sm_total_tax_amt").value;
		var postTaxAmt = parseInt(preValue) * parseInt(preVat) / 100;
		var taxAmt = parseInt(preTaxAmt) -parseInt(postTaxAmt);
		document.getElementById("sm_total_tax_amt").value = taxAmt;

		var preDiscAmt = document.getElementById("sm_disc_amt").value;
		var discRate = document.getElementById("sm_disc_rate").value;
		var postDiscAmt = parseInt(preValue) * parseInt(discRate) / 100;
		var discAmt = parseInt(preDiscAmt) -parseInt(postDiscAmt);
		document.getElementById("sm_disc_amt").value = discAmt;

		document.getElementById("sm_netamt").value = (parseInt(total) + parseInt(taxAmt)) - parseInt(discAmt) ;
	}
